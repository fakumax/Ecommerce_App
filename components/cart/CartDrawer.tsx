    ...
    <>
      ...
      <aside className="fixed right-0 top-0 z-50 flex h-full w-full max-w-md flex-col bg-surface shadow-2xl">
        <div className="flex items-center justify-between border-b border-line p-5">
          ...
          <button onClick={close} aria-label={t.cart.close} className="rounded-full p-2 text-soft hover:bg-bg">
          ...
          <>
                  ...
                  <li key={item.slug} className="flex gap-4">
                    ...
                    <div className="flex flex-1 flex-col">
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <button onClick={() => remove(item.slug)} aria-label={`${t.cart.remove} ${p.name}`} className="text-soft hover:text-ink">
                      ...
                      <div className="mt-auto flex items-center gap-3">
                        <button onClick={() => setQty(item.slug, item.qty - 1)} disabled={item.qty <= 1} aria-label={t.cart.minus} className="flex h-7 w-7 items-center justify-center rounded-full border border-line disabled:opacity-40">−</button>
                        <span className="w-6 text-center text-sm font-medium">{item.qty}</span>
                        <button onClick={() => setQty(item.slug, item.qty + 1)} aria-label={t.cart.plus} className="flex h-7 w-7 items-center justify-center rounded-full border border-line">+</button>
                      </div>
                    </div>
                  </li>
                );
            ...
            <div className="border-t border-line p-5">
              <div className="mb-4 flex items-center justify-between">
                <span className="text-soft">{t.cart.total}</span>
